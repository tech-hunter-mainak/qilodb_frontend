...
function Footer() {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        emailjs.send(
            'service_nkucyl8',           // Your EmailJS Service ID
            'template_87kiu9l',          // Your EmailJS Template ID
            templateParams,
            'F5P9bTNw9uli2GklD'          // Your EmailJS Public Key
        ).then((response) => {
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-8 py-10 max-sm:max-w-sm max-sm:mx-auto gap-y-8">
                    ...
                    <div className="col-span-full mb-10 lg:col-span-2 lg:mb-0">
                        <Link to="/" className="flex justify-center lg:justify-start">
                            <img src="favicon.ico" alt="QiloDB logo" />
                        </Link>
                    </div>
                    ...
                    <div className="lg:mx-auto text-left">
                        ...
                        <ul className="text-sm">
                            <li className="mb-6"><Link to="/" className="text-gray-600 hover:text-gray-900" onClick={() => {
                                document.getElementById('top')?.scrollIntoView({
                                    behavior: 'smooth'
                                })
                            }}>Home</Link></li>
                            <li className="mb-6"><Link to="/about" className="text-gray-600 hover:text-gray-900">About</Link></li>
                            <li className="mb-6"><Link to="/download" className="text-gray-600 hover:text-gray-900">Download</Link></li>
                            <li><Link to="/" className="text-gray-600 hover:text-gray-900" onClick={() => {
                    ...
                    <div className="lg:mx-auto text-left">
                        ...
                        <ul className="text-sm">
                            <li className="mb-6"><Link to="/" className="text-gray-600 hover:text-gray-900" onClick={()=>{document.getElementById('faqs')?.scrollIntoView({
                                behavior: 'smooth'
                            })}}>FAQs</Link></li>
                            <li className="mb-6"><Link to="/docs" className="text-gray-600 hover:text-gray-900">Documentation</Link></li>
                            <li><Link to="/" className="text-gray-600 hover:text-gray-900">User Guide</Link></li>
                        </ul>
                    </div>

                    {/* Email Signup */}
                    <div className="lg:mx-auto text-left col-span-2">
                        <h4 className="text-lg text-gray-900 font-medium mb-7">Get in touch</h4>
                        <form onSubmit={handleSubmit} className="flex flex-col">
                            <input
                                type="email"
                                value={email}
                                onChange={(e) => setEmail(e.target.value)}
                                className="shadow-md p-3 outline-0 border border-gray-500 rounded-md w-74 text-black"
                                placeholder="Enter your email"
                                required
                                name="user_email"
                            />
                            {message && <p className="text-neutral-500 mt-2">{message}</p>}
                            <button
                                type="submit"
                                className="mt-4 bg-blue-600 text-white p-3 rounded-md hover:bg-blue-700 transition duration-300"
                            >
                                Subscribe
                            </button>
                        </form>
